...
# Prefer CLAUDE_CODE_USER_EMAIL (the actual account owner in Claude Code on
# the Web) over `git config user.email`, which this harness sets to
# noreply@anthropic.com for commit verification and does not identify the
# human user.
USER_EMAIL="${CLAUDE_CODE_USER_EMAIL:-$(git config user.email 2>/dev/null || true)}"
